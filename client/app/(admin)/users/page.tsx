...
export default function UsersPage() {
  ...
  const {
    ...
    total: activeTotal,
    reset: resetActive,
  } = usePagination(activeFiltered, USERS_PER_PAGE);
  ...
  useEffect(() => {
    setActivePage(1);
  }, [search, planFilter, statusFilter, dateFrom, dateTo, sortField, sortDir, setActivePage]);
  ...
  useEffect(() => {
    resetActive();
  }, [search, planFilter, statusFilter, resetActive]);
        ...
        <div className={styles.card}>
          ...
          <div className={styles.paginationRow}>
            <span className={styles.paginationInfo}>
              Showing{" "}
              {activeTotal === 0 ? 0 : (activePage - 1) * USERS_PER_PAGE + 1}
            ...
            <div className={styles.pagination}>
              ...
              {Array.from({ length: Math.min(Math.ceil(activeTotal / USERS_PER_PAGE), 7) }, (_, i) => (
              ...
              <button
                className={styles.pageBtn}
                onClick={() => setActivePage((p) => Math.min(Math.ceil(activeTotal / USERS_PER_PAGE), p + 1))}
                disabled={activePage === Math.ceil(activeTotal / USERS_PER_PAGE)}
              >
                Next →
              </button>
            </div>
        ...
        <div className={styles.card}>
              ...
              >
                ...
                {du.deletionReason && (
                  <div className={styles.deletedReason}>
                    <span className={styles.deletedReasonLabel}>Reason:</span>{" "}
                    {du.deletionReason}
                  </div>
                )}
          ...
          <div className={styles.paginationRow}>
            <span className={styles.paginationInfo}>
              ...
              {Math.min(deletedPageNum * DELETED_PER_PAGE, deletedUsers.length)} of{" "}
              {deletedUsers.length}
            </span>
            <div className={styles.pagination}>
              <button
                className={styles.pageBtn}
                onClick={() => setDeletedPageNum((p) => Math.max(1, p - 1))}
                disabled={deletedPageNum === 1}
              >
                ← Prev
              </button>
              {Array.from({ length: Math.ceil(deletedUsers.length / DELETED_PER_PAGE) }, (_, i) => (
                <button
                  key={i}
                  className={`${styles.pageBtn} ${deletedPageNum === i + 1 ? styles.pageBtnActive : ""}`}
                  onClick={() => setDeletedPageNum(i + 1)}
                >
                  {i + 1}
                </button>
              ))}
              <button
                className={styles.pageBtn}
                onClick={() =>
                  setDeletedPageNum((p) => Math.min(Math.ceil(deletedUsers.length / DELETED_PER_PAGE), p + 1))
                }
                disabled={deletedPageNum === Math.ceil(deletedUsers.length / DELETED_PER_PAGE)}
              >
                Next →
              </button>
            </div>
            ...
            >
              ...
              <div style={{ display: "flex", gap: 10, padding: "0 20px 20px" }}>
                <button
                  ...
                  disabled={
                    !selectedNewPlan || selectedNewPlan === changePlanUser.plan
                  }
                  style={{
                    flex: 1,
                    background: "var(--blue-primary)",
